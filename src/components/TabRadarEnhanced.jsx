...
import { getIndiaWeather, hasWeatherKey, detectUserState } from '../utils/dataService';
import { useTranslation } from '../hooks/useLocalT';
import { getCalendarForState, getSowingAlert } from '../utils/cropCalendar';

const QUICK_CITIES = ['Mandya', 'Bengaluru', 'Pune', 'Nashik', 'Ludhiana', 'Indore', 'Bhopal', 'Jaipur', 'Patna', 'Kochi'];
...
export default function TabRadar({ selectedLang, isSunlightMode }) {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    if (mounted.current) {
      ...
      setLoading(false);
      setRefreshing(false);
      // Push notification logic: if rain in 3h, notify
      const rainDesc = (data?.forecast?.[0]?.description || data?.condition || '').toLowerCase();
      const rainToday = (data?.rainfallMm ?? 0) > 0 || /rain|drizzle|shower|thunder/.test(rainDesc);
      if (rainToday && pushEnabled && 'Notification' in window && Notification.permission === 'granted') {
        new Notification('🌧️ AgriPulse: Rain Alert', { body: `Rain expected in ${city} in 3 hours. Don't spray today!`, icon: '/icon-192.png' });
      }
    }
  }, [city, pushEnabled]);
  ...
  useEffect(() => {
    ...
    const autoDetectCity = async () => {
      try {
        ...
        if (info?.city) { setCity(info.city); setCityInput(info.city); setDetectedCity(info.city); }
  ...
  const handleAutoDetect = async () => {
    ...
    try {
      const info = await detectUserState();
      if (info?.city) { setCity(info.city); setCityInput(info.city); setDetectedCity(info.city); sound.playSuccess(); }
      else if (info?.state) {
        const capitals = { 'Karnataka':'Bengaluru','Maharashtra':'Pune','Punjab':'Ludhiana','Tamil Nadu':'Chennai','Gujarat':'Ahmedabad','Rajasthan':'Jaipur','Uttar Pradesh':'Lucknow','Bihar':'Patna','Kerala':'Kochi' };
        const fallback = capitals[info.state] || info.state;
        setCity(fallback); setCityInput(fallback); setDetectedCity(fallback); sound.playSuccess();
      }
      if (info?.state) setUserState(info.state);
    } catch { sound.playTransition(); } finally { setLocating(false); }
  };

  const handleCitySubmit = (e) => { e?.preventDefault(); const c = cityInput.trim(); if (!c) return; sound.playClick(); setCity(c); };

  const handleAlert = () => { sound.playSuccess(); setAlertSent(true); setTimeout(() => setAlertSent(false), 2500); };
  ...
  const handlePushToggle = async () => {
    ...
    if (Notification.permission === 'granted') {
      ...
      if (!pushEnabled) sound.playSuccess();
    } else if (Notification.permission !== 'denied') {
      const perm = await Notification.requestPermission();
      if (perm === 'granted') { setPushEnabled(true); sound.playSuccess(); new Notification('✅ AgriPulse push enabled', { body: 'You will get rain alerts: Don\'t spray today', icon: '/icon-192.png' }); }
    } else {
  ...
  const sprayEndRaw = addMinutes(wx?.sunrise, 270);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 lg:px-8 py-4 space-y-6">
      {!hasWeatherKey() && <div className="p-3.5 rounded-2xl bg-blue-500/10 border border-blue-500/40 flex items-start gap-3 text-blue-400 text-xs font-bold shadow-md"><AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" /><span>{wl.noKeyBanner}</span></div>}

      {rainToday ? <div className="p-3.5 rounded-2xl bg-amber-500/15 border border-amber-500/40 flex items-center gap-3 text-amber-500 text-xs font-bold shadow-md"><CloudRain className="w-5 h-5 shrink-0 animate-bounce" /><span>{wl.washoutYes}</span></div> : <div className="p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/40 flex items-center gap-3 text-emerald-500 text-xs font-bold shadow-md"><ShieldCheck className="w-5 h-5 shrink-0" /><span>{wl.washoutNo}</span></div>}
      ...
      <form onSubmit={handleCitySubmit} className="flex flex-wrap items-center gap-2">
        <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border-2 grow sm:grow-0 ${isSunlightMode ? 'bg-white border-zinc-300' : 'bg-zinc-900 border-zinc-700'}`}><MapPin className="w-4 h-4 text-emerald-500 shrink-0" /><input value={cityInput} onChange={(e) => setCityInput(e.target.value)} placeholder={wl.cityPlaceholder} className={`bg-transparent outline-none text-xs font-black w-36 sm:w-44 ${isSunlightMode ? 'text-zinc-900' : 'text-white'}`} /></div>
        <button type="submit" className="px-4 py-2 rounded-xl bg-emerald-400 hover:bg-emerald-300 text-black text-xs font-black border-2 border-emerald-300 shadow-sm transition-transform active:scale-95">{wl.applyCity}</button>
        <div className="flex items-center gap-1.5 overflow-x-auto max-w-full">{QUICK_CITIES.map((c) => (<button type="button" key={c} onClick={() => { sound.playClick(); setCityInput(c); setCity(c); }} className={`px-2.5 py-1.5 rounded-lg text-[11px] font-black whitespace-nowrap border transition-all ${city.toLowerCase() === c.toLowerCase() ? 'bg-emerald-400 text-black border-emerald-300' : isSunlightMode ? 'bg-white text-zinc-700 border-zinc-300' : 'bg-zinc-800 text-zinc-200 border-zinc-700'}`}>{c}</button>))}</div>
      </form>
        ...
        <div className="lg:col-span-6 space-y-4">
          <div className={`p-6 rounded-2xl border space-y-4 ${isSunlightMode ? 'bg-white border-zinc-300 text-zinc-900' : 'bg-[#121514] border-[#1f2421] text-white'}`}>
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs font-mono">
              ...
              <div className={`p-3 rounded-xl border ${isSunlightMode ? 'bg-zinc-100 border-zinc-300' : 'bg-[#181c1a] border-[#232925]'}`}><CloudRain className="w-4 h-4 text-indigo-400 mx-auto mb-1" /><div className="text-[10px] text-zinc-500">{wl.rainLabel}</div><div className="font-bold mt-0.5">{loading ? '…' : wx?.rainfallMm != null ? `${wx.rainfallMm} mm` : '0 mm'}</div></div>
              <div className={`p-3 rounded-xl border ${isSunlightMode ? 'bg-zinc-100 border-zinc-300' : 'bg-[#181c1a] border-[#232925]'}`}><Thermometer className="w-4 h-4 text-red-400 mx-auto mb-1" /><div className="text-[10px] text-zinc-500">{wl.tempMax} / {wl.tempMin}</div><div className="font-bold mt-0.5">{loading ? '…' : wx?.tempMax != null ? `${Math.round(wx.tempMax)}° / ${Math.round(wx.tempMin)}°C` : '—'}</div></div>
              <div className={`p-3 rounded-xl border ${isSunlightMode ? 'bg-zinc-100 border-zinc-300' : 'bg-[#181c1a] border-[#232925]'}`}><Sunrise className="w-4 h-4 text-amber-400 mx-auto mb-1" /><div className="text-[10px] text-zinc-500">{wl.sunrise}</div><div className="font-bold mt-0.5">{loading ? '…' : addMinutes(wx?.sunrise, 0) || '—'}</div></div>
              <div className={`p-3 rounded-xl border ${isSunlightMode ? 'bg-zinc-100 border-zinc-300' : 'bg-[#181c1a] border-[#232925]'}`}><Sunset className="w-4 h-4 text-orange-400 mx-auto mb-1" /><div className="text-[10px] text-zinc-500">{wl.sunset}</div><div className="font-bold mt-0.5">{loading ? '…' : addMinutes(wx?.sunset, 0) || '—'}</div></div>
            </div>
            <div className={`p-4 rounded-xl border ${sprayAdvised ? isSunlightMode ? 'bg-zinc-100 border-emerald-500/40' : 'bg-[#181c1a] border-emerald-500/30' : isSunlightMode ? 'bg-red-50 border-red-400' : 'bg-[#231515] border-red-500/30'}`}><div className={`text-xs font-bold flex items-center gap-1.5 ${sprayAdvised ? 'text-emerald-500' : 'text-red-500'}`}>{sprayAdvised ? <ShieldCheck className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}{sprayAdvised ? wl.bestWindow : !windSafe ? wl.windCaution : wl.washoutYes}</div>{sprayAdvised && <div className="text-lg font-bold mt-1">{sprayStart ?? '6:30 AM'} – {sprayEnd}</div>}<p className={`text-xs font-light mt-1 ${isSunlightMode ? 'text-zinc-600' : 'text-zinc-400'}`}>{wx?.condition}</p></div>
            <button onClick={handleAlert} className="w-full py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black font-bold text-xs flex items-center justify-center gap-2"><Bell className="w-4 h-4" /><span>{alertSent ? t.radar.alertSentText : t.radar.sendAlertBtn}</span></button>
          </div>
      ...
      <div className={`p-6 rounded-2xl border ${isSunlightMode ? 'bg-white border-zinc-300' : 'bg-[#121514] border-[#1f2421]'}`}>
        <h3 className={`text-sm font-bold font-mono flex items-center gap-2 mb-4 ${isSunlightMode ? 'text-zinc-900' : 'text-white'}`}><CalendarDays className="w-4 h-4 text-emerald-500" /> {wl.forecastTitle}</h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2">
          {(loading ? Array.from({ length: 7 }) : wx?.forecast || []).map((f, i) => f ? <div key={i} className={`p-3 rounded-xl border text-center ${/rain|drizzle|shower|thunder/.test((f.description || '').toLowerCase()) ? isSunlightMode ? 'bg-blue-50 border-blue-300' : 'bg-blue-950/30 border-blue-800' : isSunlightMode ? 'bg-zinc-100 border-zinc-300' : 'bg-[#181c1a] border-[#232925]'}`}><div className={`text-[10px] font-mono font-bold ${isSunlightMode ? 'text-zinc-600' : 'text-zinc-400'}`}>{f.date}</div><div className="text-lg my-1">{/rain|drizzle|shower|thunder/.test((f.description || '').toLowerCase()) ? '🌧️' : /cloud/.test((f.description || '').toLowerCase()) ? '⛅' : '☀️'}</div><div className={`text-[11px] font-black font-mono ${isSunlightMode ? 'text-zinc-900' : 'text-white'}`}>{Math.round(f.maxTemp)}° / {Math.round(f.minTemp)}°</div><div className={`text-[9px] font-mono mt-1 leading-tight ${isSunlightMode ? 'text-zinc-500' : 'text-zinc-400'}`}>{(f.description || '').length > 42 ? `${f.description.slice(0, 42)}…` : f.description}</div></div> : <div key={i} className={`p-3 rounded-xl border h-[110px] animate-pulse ${isSunlightMode ? 'bg-zinc-100 border-zinc-200' : 'bg-[#181c1a] border-[#232925]'}`} />)}
        </div>
      </div>
    </div>
  );
